...
export function Footer(): React.JSX.Element {
  return (
    <footer className="fixed inset-x-0 bottom-0 z-[100] bg-gradient-to-t from-[rgba(4,6,15,0.97)] via-[rgba(4,6,15,0.7)] to-transparent backdrop-blur-[12px]">
      <div className="mx-auto flex w-full max-w-7xl flex-wrap items-center justify-center gap-x-4 gap-y-1 px-3 py-2 text-[11px] text-white/45 sm:justify-between sm:px-5 sm:text-xs">
        ...
        {links.map((link, i) => (
          <span key={link.href} className="inline-flex items-center gap-3 sm:gap-4">
            <a
              ...
              className="text-white/45 no-underline transition-colors hover:text-white/90"
            >
              {link.label}
            </a>
            {i < links.length - 1 && (
      ...
      <div className="inline-flex items-center justify-center gap-1">
        <a
          ...
          className="inline-flex items-center justify-center p-2 text-white/65 transition-colors hover:text-white/95"
        >

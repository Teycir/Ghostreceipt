...
const DEFAULT_CHAINSTACK_THROTTLE_MS = 250;
const DEFAULT_CHAINSTACK_RETRIES = 1;
const DEFAULT_CHAINSTACK_RETRY_DELAY_MS = 250;
const RPC_SCOPE = 'provider:solana-chainstack';
...
export class SolanaChainstackProvider implements SolanaProvider {
  readonly name = 'solana-chainstack';
  readonly chain = 'solana' as const;
  readonly config: ProviderConfig = {
    name: 'solana-chainstack',
    priority: 2,
    requiresApiKey: false,
    rateLimit: {
      requestsPerSecond: 4,
      requestsPerDay: 345600,
    },
  };
  ...
  constructor() {
    this.endpointUrl = resolveRequiredEndpointUrl(
      SOLANA_PROVIDER_API_ENDPOINTS,
      'CHAINSTACK_MAINNET',
      'SOLANA_PROVIDER_API_ENDPOINTS.CHAINSTACK_MAINNET',
      SOLANA_PROVIDER_API_ENDPOINT_ENV_KEYS
    );
    ...
    if (!urlValidation.valid) {
      throw new Error(
        `[Config] Invalid Chainstack Solana RPC URL: ${this.endpointUrl} (${urlValidation.error ?? 'invalid URL'})`
      );
    }

    this.throttleMs = parseNonNegativeIntEnv(
      'SOLANA_CHAINSTACK_REQUEST_THROTTLE_MS',
      DEFAULT_CHAINSTACK_THROTTLE_MS
    );
    this.requestRetries = parseNonNegativeIntEnv(
      'SOLANA_CHAINSTACK_ENDPOINT_RETRIES',
      DEFAULT_CHAINSTACK_RETRIES
    );
    this.requestRetryDelayMs = parseNonNegativeIntEnv(
      'SOLANA_CHAINSTACK_ENDPOINT_RETRY_DELAY_MS',
      DEFAULT_CHAINSTACK_RETRY_DELAY_MS
    );
  }

  static isConfigured(env: NodeJS.ProcessEnv = process.env): boolean {
    const value = env[SOLANA_PROVIDER_API_ENDPOINT_ENV_KEYS.CHAINSTACK_MAINNET] ?? '';
  ...
  async fetchTransaction(txHash: string, signal?: AbortSignal): Promise<CanonicalTxData> {
    ...
    try {
      ...
      // Confirmation depth should not invalidate a canonical tx fetch.
  ...
  private async requestRpc<T>(
    ...
    while (true) {
      try {
        return await this.requestRpcAttempt<T>(method, params, signal);
      } catch (error) {
        ...
        attempt += 1;
        await this.waitBeforeRetry(this.requestRetryDelayMs * attempt, signal);
      }
    }
  }

  private async requestRpcAttempt<T>(
    method: string,
    params: unknown[],
    signal?: AbortSignal
  ): Promise<T> {
    await waitForProviderThrottleSlot(`${RPC_SCOPE}:${this.endpointUrl}`, this.throttleMs);
    ...
    try {
      ...
      if (error instanceof SyntaxError) {
        throw new Error('Invalid JSON response from Solana Chainstack RPC');
      }
      throw error;
    }
    ...
    if (payload.error && typeof payload.error === 'object') {
      ...
      throw new Error(`Solana Chainstack RPC error: ${errorMessage}`);
    }

    if (!Object.prototype.hasOwnProperty.call(payload, 'result')) {
      throw new Error('Malformed Solana Chainstack RPC response');
    }
